...
#After defining the MRexperiment object, the first step is to calculate the correct percentile  Normalization is important. There are several ways to calculate and visualize relative values Difference in  reference.
...
#After the user has defined the appropriate model matrix for the hypothesis test, there is an optional model matrix Input to fitZig, including the settings set by zigControl. Ask the user for confirmation help files for  fitZig and zigControl. In this example, we include the body position as follows: We would like to test for  bacteria that are covariates and have different abundances for smokers and nonsmokers.
...
## Aggregating counts
# Normalization  at the OTU level is recommended. However, there is an aggregation function A count matrix  based on a particular user-defined level (whether normalized or not). Call aggregateByTaxonomy or aggTax using the featureData information of the MRexperiment object. Declaration of MRexperiment object and specific featureData column name (ie "genus"). Use the aggfun function (default colSums) to aggregate the counts to the desired level. Possible fun Alternatives are colMeans and colMedians.
...
### Structural overview
#Many studies begin by comparing the frequency composition of the entire specimen or trait phenotype. The first step in data analysis is often a heatmap, correlation or co-occurrence plot, or something similar. Other data exploration methods. The following features are implemented: Overview of the first steps of the data:
...
# Below is an example of plotting CMDS plots of the data and the rarefaction effect at the OTU level. None of the data is removed
